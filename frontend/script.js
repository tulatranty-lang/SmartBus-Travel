...
const TravelUI = {
  ...
  async _getNearbyStopsTrietDe(gps, limit = 5) {
    ...
    // Tầng 3: Fetch file JSON tĩnh (bọc riêng – không để throw lên trên)
    try {
    ...
    // Tầng 4: Hardcoded fallback – 20 bến nhúng thẳng, không cần fetch hay backend
    try {
  ...
  // Tầng 4 hardcoded: 20 bến chính Đà Nẵng + lân cận – luôn hoạt động offline
  _getHardcodedStops() {
    return [
