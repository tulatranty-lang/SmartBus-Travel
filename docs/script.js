...
const Nav = {
  ...
  bind() {
    ...
    $("#menu-btn")?.addEventListener("click", () => this._openSidebar());
  ...
  bind() {
    ...
    $("#gis-gps-btn")?.addEventListener("click", () => {
      if (!navigator.geolocation) {
        Toast.show("Trình duyệt không hỗ trợ định vị GPS", "error");
        return;
      }
      Toast.show("Đang lấy vị trí GPS chính xác...", "info", 1800);
      navigator.geolocation.getCurrentPosition(
        (pos) => {
          const { latitude, longitude } = pos.coords;
          MapModule.markUserLocation(latitude, longitude);
          Toast.show("Đã bật vị trí của bạn trên bản đồ", "success", 2200);
          refresh();
        },
        () => {
          Toast.show(
...
const TravelUI = {
  ...
  async _getGps(showToast = false) {
    ...
    if (!navigator.geolocation) {
      if (showToast) Toast.show("Trình duyệt không hỗ trợ GPS", "warning");
      return null;
    }
    return new Promise((resolve) => {
      navigator.geolocation.getCurrentPosition(
        (pos) => {
          ...
          MapModule.markUserLocation?.(this.gps.lat, this.gps.lng);
          if (showToast) Toast.show("Đã lấy vị trí GPS", "success");
          resolve(this.gps);
        },
        () => {
          if (showToast)
...
const SmartBusAssistant = (() => {
  ...
  function requestGPS(showToast = false, timeoutMs = 6000) {
    const status = els().gpsStatus;
    if (!navigator.geolocation) {
      if (status) status.textContent = "Trình duyệt không hỗ trợ GPS";
      return Promise.resolve(null);
    }
    if (status) status.textContent = "Đang lấy vị trí GPS...";
    return new Promise((resolve) => {
