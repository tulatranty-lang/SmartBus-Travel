...
/* ============================================================
   SMARTBUS V5 CORE EXTENSION: 3 LAYER + MODULAR MONOLITH
   Không DROP dữ liệu cũ; chỉ bổ sung bảng/cột/index cần thiết.
============================================================ */
USE SmartBusDB;
GO

IF COL_LENGTH('provinces','id') IS NULL ALTER TABLE provinces ADD id INT IDENTITY(1,1);
IF COL_LENGTH('provinces','province_code') IS NULL ALTER TABLE provinces ADD province_code NVARCHAR(50) NULL;
IF COL_LENGTH('provinces','province_name') IS NULL ALTER TABLE provinces ADD province_name NVARCHAR(120) NULL;
IF COL_LENGTH('provinces','display_name') IS NULL ALTER TABLE provinces ADD display_name NVARCHAR(180) NULL;
IF COL_LENGTH('provinces','is_active') IS NULL ALTER TABLE provinces ADD is_active BIT NOT NULL CONSTRAINT DF_provinces_is_active DEFAULT 1;
GO
UPDATE provinces SET province_code=COALESCE(province_code, code), province_name=COALESCE(province_name, name), display_name=COALESCE(display_name, name), is_active=COALESCE(is_active, 1);
GO

IF COL_LENGTH('bus_routes','route_name') IS NULL ALTER TABLE bus_routes ADD route_name NVARCHAR(255) NULL;
IF COL_LENGTH('bus_routes','start_point') IS NULL ALTER TABLE bus_routes ADD start_point NVARCHAR(500) NULL;
IF COL_LENGTH('bus_routes','end_point') IS NULL ALTER TABLE bus_routes ADD end_point NVARCHAR(500) NULL;
IF COL_LENGTH('bus_routes','route_summary') IS NULL ALTER TABLE bus_routes ADD route_summary NVARCHAR(MAX) NULL;
IF COL_LENGTH('bus_routes','operating_hours') IS NULL ALTER TABLE bus_routes ADD operating_hours NVARCHAR(1000) NULL;
IF COL_LENGTH('bus_routes','frequency_text') IS NULL ALTER TABLE bus_routes ADD frequency_text NVARCHAR(500) NULL;
IF COL_LENGTH('bus_routes','estimated_vehicle_count') IS NULL ALTER TABLE bus_routes ADD estimated_vehicle_count INT NULL;
IF COL_LENGTH('bus_routes','speed_range_text') IS NULL ALTER TABLE bus_routes ADD speed_range_text NVARCHAR(500) NULL;
IF COL_LENGTH('bus_routes','break_time_text') IS NULL ALTER TABLE bus_routes ADD break_time_text NVARCHAR(1000) NULL;
IF COL_LENGTH('bus_routes','source_text') IS NULL ALTER TABLE bus_routes ADD source_text NVARCHAR(2000) NULL;
GO
UPDATE bus_routes SET route_name=COALESCE(route_name,name), start_point=COALESCE(start_point,origin_name), end_point=COALESCE(end_point,destination_name), route_summary=COALESCE(route_summary,description), operating_hours=COALESCE(operating_hours,operating_time), frequency_text=COALESCE(frequency_text,interval_text), estimated_vehicle_count=COALESCE(estimated_vehicle_count,vehicle_count), speed_range_text=COALESCE(speed_range_text,speed_note), break_time_text=COALESCE(break_time_text,status_text), source_text=COALESCE(source_text,source_name);
GO

IF OBJECT_ID('bus_schedules','U') IS NULL
BEGIN
    CREATE TABLE bus_schedules(
        id INT IDENTITY(1,1) PRIMARY KEY,
        route_code NVARCHAR(50) NOT NULL,
        direction NVARCHAR(120) NULL,
        trip_label NVARCHAR(120) NULL,
        first_departure NVARCHAR(30) NULL,
        last_departure NVARCHAR(30) NULL,
        departure_times_sample NVARCHAR(MAX) NULL,
        arrival_times_sample NVARCHAR(MAX) NULL,
        frequency_text NVARCHAR(500) NULL,
        notes NVARCHAR(MAX) NULL,
        source_text NVARCHAR(2000) NULL,
        created_at DATETIME2 NOT NULL DEFAULT SYSDATETIME(),
        CONSTRAINT FK_bus_schedules_routes FOREIGN KEY(route_code) REFERENCES bus_routes(route_code)
    );
END
GO
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name='IX_bus_schedules_route' AND object_id=OBJECT_ID('bus_schedules')) CREATE INDEX IX_bus_schedules_route ON bus_schedules(route_code);
GO

IF COL_LENGTH('bus_vehicles','province_code') IS NULL ALTER TABLE bus_vehicles ADD province_code NVARCHAR(50) NULL;
IF COL_LENGTH('bus_vehicles','vehicle_name') IS NULL ALTER TABLE bus_vehicles ADD vehicle_name NVARCHAR(120) NULL;
IF COL_LENGTH('bus_vehicles','vehicle_type') IS NULL ALTER TABLE bus_vehicles ADD vehicle_type NVARCHAR(80) NULL;
IF COL_LENGTH('bus_vehicles','license_plate') IS NULL ALTER TABLE bus_vehicles ADD license_plate NVARCHAR(30) NULL;
GO
UPDATE bus_vehicles SET license_plate=COALESCE(license_plate,plate), vehicle_name=COALESCE(vehicle_name,vehicle_code), vehicle_type=COALESCE(vehicle_type,N'Xe buýt'), province_code=COALESCE(province_code,(SELECT TOP 1 province_code FROM bus_routes r WHERE r.route_code=bus_vehicles.route_code));
GO

IF OBJECT_ID('place_bus_mappings','U') IS NULL
BEGIN
    CREATE TABLE place_bus_mappings(
        id INT IDENTITY(1,1) PRIMARY KEY,
        place_id INT NOT NULL,
        nearest_stop_id INT NULL,
        route_code NVARCHAR(50) NULL,
        suggested_route_text NVARCHAR(255) NULL,
        nearest_stop_text NVARCHAR(255) NULL,
        transfer_instruction NVARCHAR(MAX) NULL,
        distance_km_text NVARCHAR(80) NULL,
        confidence_level NVARCHAR(80) NULL,
        created_at DATETIME2 NOT NULL DEFAULT SYSDATETIME(),
        CONSTRAINT FK_place_bus_mappings_places FOREIGN KEY(place_id) REFERENCES tourist_places(id),
        CONSTRAINT FK_place_bus_mappings_stops FOREIGN KEY(nearest_stop_id) REFERENCES bus_stops(id)
    );
END
GO
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name='IX_place_bus_mappings_place_route' AND object_id=OBJECT_ID('place_bus_mappings')) CREATE INDEX IX_place_bus_mappings_place_route ON place_bus_mappings(place_id, route_code);
GO
...
BEGIN
    CREATE TABLE chatbot_knowledge(
        ...
        intent_code NVARCHAR(120) NOT NULL,
        question_pattern NVARCHAR(500) NOT NULL,
        answer_template NVARCHAR(MAX) NOT NULL,
...
END
GO
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name='IX_chatbot_knowledge_intent' AND object_id=OBJECT_ID('chatbot_knowledge')) CREATE INDEX IX_chatbot_knowledge_intent ON chatbot_knowledge(intent_code, is_active, priority);
GO

IF OBJECT_ID('analytics','U') IS NULL
BEGIN
...
-- Đồng bộ place_bus_mappings từ bảng place_nearby_stops hiện có nếu có dữ liệu.
INSERT INTO place_bus_mappings(place_id, nearest_stop_id, route_code, suggested_route_text, nearest_stop_text, transfer_instruction, distance_km_text, confidence_level)
SELECT pns.place_id, pns.stop_id, pns.route_code, pns.route_display_code, s.name, pns.note, CONCAT(CAST(CAST(pns.distance_meters AS FLOAT)/1000 AS DECIMAL(10,2)), N' km'), pns.reliability_level
FROM place_nearby_stops pns
LEFT JOIN bus_stops s ON s.id=pns.stop_id
WHERE NOT EXISTS (SELECT 1 FROM place_bus_mappings m WHERE m.place_id=pns.place_id AND ISNULL(m.nearest_stop_id,-1)=ISNULL(pns.stop_id,-1) AND ISNULL(m.route_code,'')=ISNULL(pns.route_code,''));
GO

PRINT 'SmartBus V5 core schema extension is ready.';
GO
