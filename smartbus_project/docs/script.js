...
const TravelUI = {
  ...
  async _getNearbyStopsTrietDe(gps, limit = 5) {
    // Tầng 1: Backend API (Render có thể cold start, tăng timeout)
    try {
      ...
      if (stops.length) {
        return {
          stops: stops.map((stop, i) => this._normalizeNearbyStop(stop, gps, i)).filter(Boolean),
    ...
    // Tầng 4: Hardcoded fallback — 20 bến Đà Nẵng nhúng thẳng vào code, không cần fetch
    try {
      ...
      <div class="travel-card wide admin-shell">
        <div class="admin-toolbar">
          <div>
            ...
            <p>Tập trung duyệt review cộng đồng thật từ backend; user thường không có quyền truy cập.</p>
          </div>
    ...
    try {
      ...
      box.innerHTML = `
        <div class="admin-filters">
          ...
          <select id="admin-review-status" class="travel-input"><option value="pending">Chờ duyệt</option><option value="approved">Đã duyệt</option><option value="approved_seed">Bài mẫu</option><option value="rejected">Đã từ chối</option><option value="all">Tất cả</option></select>
