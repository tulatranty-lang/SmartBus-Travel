...
async function generate(input, user = null, save = true) {
  // FIX: Giới hạn số candidates để không query quá nặng
  const interests = inferInterests(input.interests);
  const categoryMap = {
    'biển': 'beach', 'van hoa': 'culture', 'văn hóa': 'culture',
    'check-in': 'checkin', 'mua sắm': 'shopping', 'vui chơi': 'entertainment',
    'tâm linh': 'spiritual', 'tiết kiệm': '',
  };

  // FIX: Chỉ lấy tối đa 12 ứng viên thay vì 16, giảm N+1
  const province = input.province || input.provinceCode || null;
  const cacheKey = `trip:recommended:${province}:${interests.join(',')}`;
  let candidates = cache.get(cacheKey);
  if (!candidates) {
    candidates = await tourism.recommended({
      lat: input.lat,
      lng: input.lng,
      province,
      limit: 12, // giảm từ 16 xuống 12
      mood: interests.join(' '),
      time: input.timeAvailable,
    });
    // Cache 3 phút cho cùng tỉnh + sở thích
    cache.set(cacheKey, candidates, 3 * 60 * 1000);
  }

  const wanted = interests.map((i) => categoryMap[i]).filter(Boolean);
  if (wanted.length) {
    candidates = candidates
      .filter((p) => wanted.includes(p.category) || p.category === 'food')
      .concat(candidates.filter((p) => !wanted.includes(p.category)));
  }
  ...
  for (const cat of wantedOrder) {
    const found = candidates.find((p) =>
      !balanced.some((x) => x.id === p.id) &&
      (p.category === cat || (cat === 'food' && /ăn|food|ẩm thực/i.test(`${p.categoryName || ''} ${p.foodSuggestions || ''}`)))
    );
  ...
  const limit = chooseLimit(input.timeAvailable);
  const items = balanced.slice(0, limit).map((p, idx) => ({
    ...p,
    order: idx + 1,
    ...
    walkingMinutes: p.nearestStop?.walkingMinutes || p.walkingMinutesFromUser,
    practicalNote: idx === 1
      ? 'Ưu tiên ăn uống/nghỉ nhẹ, không nhồi thêm quá nhiều điểm.'
      : 'Sắp xếp theo cụm gần tuyến để giảm di chuyển vòng.',
