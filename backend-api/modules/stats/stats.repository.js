...
async function overview({ includeAdmin = false } = {}) {
  const [totalRoutes, totalStops, totalVehicles, totalTourismPlaces, totalReviews, pendingReviews, totalCommunityPosts, totalReports, totalTripPlans, totalUsers, totalProvinces] = await Promise.all([
    ...
    scalar(`SELECT COUNT(1) AS total FROM community_reviews WHERE COALESCE(status, 'approved') IN ('approved','approved_seed')`),
    scalar(`SELECT COUNT(1) AS total FROM community_reviews WHERE status = 'pending'`),
    ...
    scalar(`SELECT COUNT(1) AS total FROM trip_plans`),
