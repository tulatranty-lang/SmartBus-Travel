...
function pointFeature(item, type) {
  ...
  // GeoJSON luôn dùng [lng, lat], còn frontend Leaflet luôn chuyển về [lat, lng].
...
async function stopsGeoJson(filters = {}) {
  return cache.remember(`map:stops:${JSON.stringify(filters)}`, 45_000, async () => {
    ...
    return { type: 'FeatureCollection', features: stops.map((s) => pointFeature(s, 'bus_stop')).filter(Boolean) };
  });
}
async function tourismGeoJson(filters = {}) {
  return cache.remember(`map:tourism:${JSON.stringify(filters)}`, 45_000, async () => {
    ...
    return { type: 'FeatureCollection', features: places.map((p) => pointFeature({ ...p, lat: p.lat || p.latitude, lng: p.lng || p.longitude }, 'tourist_place')).filter(Boolean) };
  });
}
async function vehiclesLayer(filters = {}) {
  const vehicles = await data.getVehicles(filters || {}).catch(() => []);
  return { type: 'FeatureCollection', features: vehicles.map((v) => pointFeature(v, 'vehicle')).filter(Boolean) };
}
