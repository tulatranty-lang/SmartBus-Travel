...
if (isProduction) {
  ...
  if (missing.length) {
    throw new Error(
      `Thiếu biến môi trường production bắt buộc: ${missing.join(", ")}`,
    );
  }
}
...
if (
  !isProduction &&
  (!accessSecret || accessSecret === "change_me_access_secret")
) {
  // Cảnh báo nhưng không chặn môi trường demo/dev.
  console.warn(
    "[SmartBus] JWT_ACCESS_SECRET đang dùng giá trị development. Hãy đổi trong production.",
  );
}
...
const env = {
  ...
  apiPrefix,
  frontendOrigins: csv(corsOrigin, [
    "http://localhost:3000",
    "http://127.0.0.1:5500",
    "http://localhost:5500",
    "https://tulatranty-lang.github.io",
  ]),
  jwtAccessSecret: accessSecret || "dev_only_smartbus_access_secret_change_me",
  jwtRefreshSecret:
    refreshSecret || "dev_only_smartbus_refresh_secret_change_me",
